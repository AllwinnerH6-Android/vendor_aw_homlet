...
void DeviceControler::present()
{
    ...
    android::sp<android::IHWCPrivateService> hwcservice = android::HWCProxy::getHwcService();
    if (hwcservice->setMargin(mLogicalId,
            margin.left, margin.right, margin.top, margin.bottom) != 0) {
        dd_error("[%s] setDisplayMargin: set margin error", mDeviceName);
    }

    /* Unblank and preset */
    hwcservice->blank(mLogicalId, 0);
...
int DeviceControler::setDisplayMode(int mode)
{
    ...
    int ret = 0;
    android::sp<android::IHWCPrivateService> hwcservice = android::HWCProxy::getHwcService();
    hwcservice->blank(mLogicalId, 1);
    ret = setDisplayModeImpl(mode);

    if (!ret) {
        /* Notify HWC to update screen window */
        hwcservice->setOutputMode(mLogicalId, mType, mode);
    }
    hwcservice->blank(mLogicalId, 0);
...
int DeviceControler::setAspectRatio(int ratio)
{
    ...
    android::sp<android::IHWCPrivateService> hwcservice = android::HWCProxy::getHwcService();
    if (hwcservice->setVideoRatio(mLogicalId, ratio) != 0) {
...
int DeviceControler::set3DLayerMode(int mode)
{
    ...
    if (mType == DISP_OUTPUT_TYPE_HDMI) {
	    ...
	    hwcservice->set3DMode(mLogicalId, mode);
	} else {
		hwcservice->set3DMode(mLogicalId, mode);
	...
	m3dLayerMode = mode;
	    ...
	    hwcservice->set3DMode(mLogicalId, m3dLayerMode);
    }
...
{
    android::sp<android::IHWCPrivateService> hwcservice = android::HWCProxy::getHwcService();
    hwcservice->blank(mLogicalId, blank);
}

void DeviceControler::hwcSetDataspace(int dataspace)
{
    android::sp<android::IHWCPrivateService> hwcservice = android::HWCProxy::getHwcService();
    hwcservice->setDataspace(mLogicalId, dataspace);
